...
void SetSubStateUser(u8 index, u8 value) {
    if (value > 0) {
        build_keyscale();
    }
}

void SetSubStateSends(u8 index, u8 value) {
    if (value > 0) {
        build_midi_channels();
    }
}

void SetSubStateAftertouch(u8 index, u8 value) {
    if (value > 0) {
        build_aftertouch();
    }
}
...
void state_keyscale(StateEvent msg, u8 index, u8 value) {
  switch (msg) {
    case EVENT_EXIT:
      exit_surface();
      hal_write_flash(0, memory_store, 30); // smart place to save here because when we leave this State often is after lots of settings changed
      break;
    case EVENT_ENTER:
